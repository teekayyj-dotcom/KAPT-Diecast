...
export function EditPoster() {
  ...
  useEffect(() => {
    const fetchPoster = async () => {
      try {
        ...
        if (!response.ok) {
          throw new Error("Failed to load poster info");
        }
        const data = await response.json();
        setName(data.name || "");
        setLocation(data.location || "Homepage Hero");
  ...
  const handleSavePoster = async (e) => {
    ...
    try {
      ...
      if (!response.ok) {
        ...
        throw new Error(data.detail || "Failed to update poster.");
      }
      ...
      if (imageFile) {
        ...
        const imageResponse = await fetch(`${apiBaseUrl}/posters/${id}/image`, {
          method: "POST",
          body: imageFormData,
        });
        ...
        if (!imageResponse.ok) {
          ...
          throw new Error(imageData.detail || "Poster updated but image upload failed.");
        }
      }

      setSuccess(`Poster #${id} updated successfully.`);
    } catch (err) {
  ...
  if (isLoading) {
    return <div className="text-center py-10 text-gray-500">Loading poster...</div>;
  }
    ...
    <form onSubmit={handleSavePoster} className="space-y-6 max-w-5xl mx-auto">
      ...
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-4">
          ...
          <div>
            <h2 className="text-2xl font-bold text-black">Edit Poster #{id}</h2>
            <p className="text-sm text-gray-500 mt-1">Update poster details and image.</p>
          </div>
        </div>
        <div className="flex items-center gap-3">
          ...
          <button type="submit" disabled={isSaving} className="bg-red-600 hover:bg-red-700 text-white px-5 py-2.5 rounded-lg text-sm font-medium transition-colors shadow-sm shadow-red-600/20 disabled:opacity-60">
            {isSaving ? "Updating..." : "Update Poster"}
          </button>
        </div>
      </div>
