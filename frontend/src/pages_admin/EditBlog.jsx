  ...
  useEffect(() => {
    const fetchBlog = async () => {
      try {
        ...
        if (!response.ok) {
          throw new Error("Failed to load blog info");
        }
        ...
        if (data.published_date) {
            setPublishedDate(new Date(data.published_date).toISOString().slice(0, 16));
        }
  ...
  const handlePublish = async (e) => {
    ...
    try {
      ...
      if (!response.ok) {
        ...
        throw new Error(data.detail || "Failed to update blog post.");
      }
      ...
      if (imageFile) {
        ...
        const imageResponse = await fetch(`${apiBaseUrl}/blogs/${id}/image`, {
          method: "POST",
          body: imageFormData,
        });
        ...
        if (!imageResponse.ok) {
          ...
          throw new Error(imageData.detail || "Blog updated but image upload failed.");
        }
      }

      setSuccess(`Post #${id} updated successfully.`);
    } catch (err) {
    ...
    <div className="space-y-6 max-w-6xl mx-auto pb-10">
      <form onSubmit={handlePublish} className="space-y-6">
        ...
        <div className="flex flex-col sm:flex-row sm:items-start sm:justify-between gap-4">
          <div>
            <h2 className="text-2xl font-bold text-black mb-1">Edit Post #{id}</h2>
            <div className="flex items-center text-sm text-gray-500 gap-2 mt-1">
              ...
              <ChevronRight size={14} className="text-gray-400" />
              <span className="text-black font-medium">Edit Post</span>
            </div>
          </div>
            ...
            <div className="lg:col-span-2 space-y-6">
              ...
              <div>
                ...
                <input
                  ...
                  className="w-full px-4 py-2.5 bg-gray-50 border border-gray-100 rounded-lg text-sm text-gray-500 font-mono"
                />
              </div>
              ...
              <div>
                ...
                <div className="relative">
                  <input
                    ...
                    className="w-full py-2.5 px-4 bg-white border border-gray-200 rounded-lg text-sm text-black focus:outline-none focus:ring-1 focus:ring-red-600 focus:border-red-600 transition-all cursor-pointer"
                  />
                </div>
              </div>
